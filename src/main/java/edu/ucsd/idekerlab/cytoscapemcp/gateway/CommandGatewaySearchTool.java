...
public class CommandGatewaySearchTool {
    ...
    private static final String TOOL_DESCRIPTION =
            "Search the full catalog of Cytoscape Desktop commands registered on user's machine"
                    + "  using a Lucene full-text query. "
                    + " WHEN TO USE: Call this tool whenever the current user conversation refers to"
                    + " Cytoscape Desktop tasks and no other registered cytoscape tools match up closely enough to the task. "
                    + " This tool will search within Cytoscape desktop for any commands that closely match up with search terms related to the task the user wants to accomplish. "
                    + " It essentially provides a dynamic bridge of tools which represent underlying desktop commands. "
                    + " Submit a Lucene-formatted query built from key terms extracted from"
                    + " the user's current request context; the tool returns a ranked list of matching"
                    + " commands with relevance scores so you can quickly identify the best"
                    + " candidates of commands.\n\n"
                    + " Call this tool proactively as the search"
                    + " is fast, light, and designed for repeated calls as the conversation evolves."
                    ...
                    + " metadata. After reviewing scores and summaries and deciding upon which command to invoke, it is required to invoke the command schema retrieval"
                    + " tool first for a commandKey from search results to get the full schema and description of the command's required and optional input parameters before invoking.\n\n"
